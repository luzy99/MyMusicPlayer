...
#include<QDebug>
MyListListWidget::MyListListWidget(QWidget *parent) : QListWidget(parent)
{
...
void MyListListWidget::contextMenuEvent(QContextMenuEvent *event)
{
    if(this->itemAt(mapFromGlobal(QCursor::pos())) != NULL)
    {
        ...
        QAction* deleteAction = new QAction(QIcon(":/icon/res/delete.png"),"删除歌单",this);
        QAction* renameAction = new QAction(QIcon(":/icon/res/rename.png"), "重命名",this);
        QAction* clearAction = new QAction(QIcon(":/icon/res/clear.png"), "清空歌单", this);
        ...
        connect(clearAction ,SIGNAL(triggered()) ,this, SLOT(clearAction_slot()));
        connect(deleteAction ,SIGNAL(triggered()) ,this, SLOT(deleteAction_slot()));
        connect(renameAction ,SIGNAL(triggered()) , this, SLOT(renameAction_slot()));
