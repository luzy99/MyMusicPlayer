...
GhostLyricWidget::GhostLyricWidget(QWidget *parent)
    :QWidget(parent)
{
    m_strCurLrc="";
    m_nFontPixSize=20;
...
GhostLyricWidget::~GhostLyricWidget()
{

}

QString GhostLyricWidget::GetLrcByIndex(int index)
{
    return m_lineMap.values().value(index);
}

qint64 GhostLyricWidget::GetPosByindex(int index)
{
     return m_lineMap.keys().value(index);
}


QString GhostLyricWidget::GetLrcByTime(qint64 time)
{
    int index=GetIndexByTime(time);
    return m_lineMap.values().value(index);
}

int GhostLyricWidget::GetIndexByTime(qint64 time)
{
    //binary search
...
void GhostLyricWidget::GetMaskLen(int nFontSize)
{
...
void GhostLyricWidget::clearLrc()
{
    m_word_list.clear();
...
void GhostLyricWidget::getPosInfo(int &keyTime, int &interval, float &precent, QString &str)
{
...
void GhostLyricWidget::setOriginalStatus()
{
    clearLrc();
    m_strCurLrc="";
    m_nCurPos=0;
...
//重定义事件的虚函数
void GhostLyricWidget::paintEvent(QPaintEvent *event)
{
    //一行歌词的信息
...
void GhostLyricWidget::slot_timer()
{
...
void GhostLyricWidget::positionChanged(qint64 length)//这应该是进度条触发后处理的槽函数
{
    m_nCurPos=length;
...
void GhostLyricWidget::DrawItem(QPainter &Painter, ItemInfo &Info)
{
...
void GhostLyricWidget::analyzeLrcContent(const QString& song_id)
{
    clearLrc();
