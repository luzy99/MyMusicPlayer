...
/*
歌单高级要求：
1.歌单处显示目前正在播放的歌单  (等双击播放列表中的歌功能实现)
2.创建或重命名歌单的时候，出现重名需要弹出提示框
3.设置我喜欢的音乐歌单为权限最高，不可更改，否则弹出提示
*/


/*
歌曲列表操作：
1.设计列表项目样式->在项前面加上喜欢或不喜欢的icon->双击播放->播放的歌曲显示图标       (最好可以点击图标切换)
2.右击item->菜单栏出现->实现移除歌曲(删除功能可以考虑)，设置为我喜欢与否， 添加到歌单(弹窗)，歌曲信息(弹窗)  (若是能在item中添加按钮，效果最好)
3.设置按钮->点击按钮为当前选中个歌单添加歌曲->打开文件视图->获取文件名字，url ，如果此歌曲存在于我喜欢的音乐，设置为1，否则为0->添加到数据库里->再显示该列表
4.注意添加歌曲不要重复，提示
5.歌曲添加或移除我喜欢的音乐时，该歌曲的喜欢属性要变
*/
...
void SongList::showSongsOfList(QString listName)
{
    ...
    QSqlQuery query(db);
    //从数据库读取数据
    QString tempcommand = QString("select songName,likeOrNot from %1");
...
//设置歌曲为我喜欢  (存在漏洞：取消和设置时，其他音乐歌单要同步。)
void SongList::setSongLiked(QString name)
{
    ...
    QString insertCommand = "insert into %1 values('%2', '%3', '%4', '%5', '%6');";

    query1.exec(insertCommand.arg(addedSonglist , name, songUrl, likeOrNot , artist, album));
...
void SongList::setSongAddInto(QString name)
{
    ...
    query.exec(insertCommand.arg(addedSonglist , name, songUrl, likeOrNot , artist, album));
    showSongsOfList(actingSongListName);
}
